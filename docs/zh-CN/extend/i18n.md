...
PrismUI 中对多语言的支持有两方面：

1. PrismUI 内部组件的多语言，比如日期组件中的日期
...
在渲染 PrismUI 组件的时候设置 locale 为 en-US
...
PrismUI 渲染的 env 参数可以配置全局文本替换，能基于它实现多语言替换功能
...
let amisScoped = amis.embed(
  ...
  {
    replaceText: {
      PRISMUI_HOST: 'https://example.com/prismui'
    }
  }
);
```

比如下面的例子会对 `PRISMUI_HOST` 进行替换
...
如果想扩展其他语言，首先参考 `https://github.com/Gengxin-Tech/prismui/blob/master/packages/prismui-ui/src/locale/en-US.ts` 文件，然后参考后面的示例注册新语言，未翻译的文字都将使用中文。
...
```javascript
import {registerLocale} from 'prismui-framework';
registerLocale('jp', {
  'Form.submit': '送信'
});
```
