...
如果发现 PrismUI 有不满足的功能，除了提 issue 等待维护者升级之外，最快的方法就是自己实现它。本文将介绍 PrismUI 代码的基本结构，并一步步教你如何新增功能。

## 准备开始

1. 首先，你需要对 React 有基本了解，快速看一遍[官方文档](https://zh-hans.react.dev/learn)即可。
2. 在 GitHub 上 fork PrismUI 项目到自己的账号下。
3. 创建分支 `git checkout -b feat-xxx`

## PrismUI 代码结构

PrismUI 主要代码在 `src` 和 `scss` 目录下，这里主要介绍 `src` 下的结构：

```
.
├── Root.tsx            // PrismUI 最外层的渲染组件
├── RootRenderer.tsx    // 用于 Root.tsx，拆分一下避免文件太大
├── Schema.ts           // 基础类型定义
├── SchemaRenderer.tsx  // PrismUI 组件渲染的主要入口
├── Scoped.tsx          // 数据域的实现
...
├── factory.tsx         // PrismUI 渲染的总入口，对外暴露的 render 方法就在这里
├── icons               // 内置的 icon
├── index.tsx           // PrismUI 项目对外的主要入口
├── locale              // 多语言翻译，目前只有英文和德语
├── locale.tsx          // 多语言支持的实现
├── renderers           // 将组件注册到 PrismUI 渲染器中，这里大部分组件是直接引用 components 里的对应组件，也有部分简单的组件是直接在这里实现，
├── store               // mst 存储
├── theme.tsx           // 主题相关的实现
├── themes              // 主题配置，有些主题效果不太好通过 css 实现，就放这了
├── types.ts            // 一些公共的类型定义
└── utils               // PrismUI 内部用的常用工具方法
```
...
本文的目标是新增 avatar 头像组件，完整演示如何在 PrismUI 中添加一个新组件。历史实现可以参考上游这个 [PR](https://github.com/baidu/amis/pull/1684/files)，本文基于这个例子进行了简化。
...
import {Renderer, RendererProps, AMISSchemaBase} from 'prismui-core';
...
这样就在 PrismUI 中新增了一个组件，接下来我们编写组件所需的样式。
...
PrismUI 中的 CSS 命名使用 [SUIT](https://github.com/suitcss/suit/blob/master/doc/naming-conventions.md) 规范，请按照这个规范编写。
...
然后通过系统提示的 PR 链接创建 Pull Request，维护者会尽快回复。
