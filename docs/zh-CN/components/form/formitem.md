...
PrismUI 会有默认的报错信息，如果你想自定义校验信息，配置`validationErrors`属性
...
默认的校验信息如下，可以直接配置文字，也可用多语言中的 key。参考：https://github.com/Gengxin-Tech/prismui/blob/master/packages/prismui-ui/src/locale/zh-CN.ts#L250
...
通过配置 "autoFill.api" 为自动填充数据源接口地址；PrismUI 可以将返回数据自动填充到表单中，如下配置；
...
自动填充接口返回格式如下：
注意：PrismUI 仅处理接口返回结果只有一项的数据，默认自动填充相关字段
