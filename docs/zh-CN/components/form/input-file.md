...
上例中，选择任意文件，然后观察数据域变化；点击提交，PrismUI 自动会调整接口数据格式为`FormData`
...
| useChunk           | `boolean`或`"auto"`            | `"auto"`                                                                                                   | PrismUI 所在服务器，限制了文件上传大小不得超出 10M，所以 PrismUI 在用户选择大文件的时候，自动会改成分块上传模式。                          |
