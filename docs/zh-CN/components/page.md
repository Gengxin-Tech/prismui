...
Page 组件是 PrismUI 页面 JSON 配置中顶级容器组件，是整个页面配置的入口组件。
...
通过设置 CSS 变量几乎可以修改 PrismUI 中任意组件的展现，具体细节请参考[样式](../../../style)。
...
虽然 PrismUI 提供了很多内置样式，但想要更精细控制样式，最好的方式依然是编写自定义 CSS，在之前的版本中需要外部页面配合，从 1.3.0 开始 PrismUI 可以直接在配置中支持自定义 CSS
