...
1. `type` 是 PrismUI 节点中最重要的字段，它告诉 PrismUI 当前节点需要渲染的是`Page`组件。
...
上面提到，`type`字段会告诉 PrismUI 当前节点渲染的组件为`Page`，组件节点的配置永远都是由 **`type`字段** （用于标识当前是哪个组件）和 **属性** 构成的。
...
除了 `Page` 之外，还有很多**容器型**的组件都有 `body` 属性，通过这种树形结构，PrismUI 就能实现复杂页面制作。

> **注意：**
>
> `Page`是 PrismUI 页面配置中 **必须也是唯一的顶级节点**
...
> PrismUI 后续将会实现新的布局模式，将更容易实现复杂布局效果
