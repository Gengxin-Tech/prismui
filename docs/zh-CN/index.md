...
为了实现用最简单方式来生成大部分页面，PrismUI 的解决方案是基于 [JSON](https://baike.baidu.com/item/JSON) 来配置，它的独特好处是：

- **不需要懂前端**：PrismUI 继承了大量复杂后台场景的实践经验，让之前从来没写过前端页面、也不会 `JavaScript` 的使用者，也能做出专业且复杂的后台界面；
- **不受前端技术更新的影响**：原项目中最早的一批页面已经持续运行多年，而当年的 `Angular/Vue/React` 版本现在都废弃了，当年流行的 `Gulp` 也被 `Webpack` 取代了，如果这些页面不是用 UI Schema 描述，现在的维护成本会很高；
- **享受 PrismUI 的不断升级**：PrismUI 一直在提升细节交互体验，比如表格首行冻结、下拉框大数据下不卡顿等，之前的 JSON 配置完全不需要修改；
- 可以 **完全** 使用 [可视化页面编辑器](../editor) 来制作页面：一般前端可视化编辑器只能用来做静态原型，而 PrismUI 可视化编辑器做出的页面是可以直接上线的。
