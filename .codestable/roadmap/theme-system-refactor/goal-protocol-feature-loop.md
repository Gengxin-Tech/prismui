...
- 再次运行 `python3 <cs-onboard skill 目录>/tools/codestable-workflow-next.py epic --roadmap .codestable/roadmap/theme-system-refactor --json`；只有结果为 `awaiting` / `dispatch_goal` 且 evidence 同时返回 `approval-report.md#goal-acceptance` 与 `approval-report.md#goal-commits`，`commitAuthorized` 才成立。其他结果先把 feature 保持 `accepted`、持久化 handoff，并停止，不得执行 `git commit`。
